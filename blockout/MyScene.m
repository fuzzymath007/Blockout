...
-(void)didBeginContact:(SKPhysicsContact *)contact {
    ...
    if (notTheBall.categoryBitMask == brickCategory) {
        self.playBrickHit = [SKAction playSoundFileNamed:@"brickhit.caf" waitForCompletion:NO];
        [self runAction:_playBrickHit];
        [notTheBall.node removeFromParent];
        _score++;
        NSLog(@"the score is %zd",_score);
        
    }
    
    if (notTheBall.categoryBitMask == paddleCategory) {
        self.playBlip = [SKAction playSoundFileNamed:@"blip.caf" waitForCompletion:NO];
        [self runAction:_playBlip];
    ...
    if (notTheBall.categoryBitMask == bottomEdgeCategory) {
        ...
        [self.view presentScene:end transition:[SKTransition doorsOpenVerticalWithDuration:0.5]];
        // end quickly[self.view presentScene:end];
    }
...
- (void)addBall:(CGSize)size {
    ...
    // create the vector kick the ball in the pants
...
-(void) addBricks:(CGSize) size {
    for (int i = 0; i < 4; i++) {
        ...
        SKSpriteNode *brick = [SKSpriteNode spriteNodeWithColor:[SKColor blueColor] size:CGSizeMake(50, 10)];
    ...
    for (int i = 0; i < 5; i++) {
        ...
        int xPos = size.width/6 * (i+1);
        int yPos = size.height - 100;
        brick.position = CGPointMake(xPos, yPos);
